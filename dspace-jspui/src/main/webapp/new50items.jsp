...
<%
	...
	if (items != null) {
		for (Item item : items) {
			org.dspace.content.DCValue[] dcv = item.getMetadata("dc", "title", null, Item.ANY);
			...
			org.dspace.content.DCValue[] dcvTypes = item.getMetadata("dc", "type", null, Item.ANY);
			String displayType = "Unknown";
			if (dcvTypes != null)
			{
				if (dcvTypes.length > 0)
				{
					displayType = org.dspace.app.webui.jsptag.ItemTag.getTypeLocalized(dcvTypes[0].value, locale);
				}
			}
