        ...
        for (int i = 0; i < fieldCount; i++)
        {
            ...
            if (i % 3 == 0) {
                script.append("  function selectItem" + (i / 3) + " (li) { " +
                        "    $(\"#" + fieldName + "_last_" + (i + 1) + "\").val(li.extra[5]); " +
                        "    $(\"#" + fieldName + "_first_" + (i + 1) + "\").val(li.extra[6]); " +
                        "    $(\"#" + fieldName + "_last_" + (i + 2) + "\").val(li.extra[3]); " +
                        "    $(\"#" + fieldName + "_first_" + (i + 2) + "\").val(li.extra[4]); " +
                        "    $(\"#" + fieldName + "_last_" + (i + 3) + "\").val(li.extra[1]); " +
                        "    $(\"#" + fieldName + "_first_" + (i + 3) + "\").val(li.extra[2]); " +
                        "  } ");
            }

            script.append("  $(document).ready(function(){ " +
                    " $(\"#" + last.toString() + "\").autocomplete(\"autocomplete.jsp\", { delay:10, minChars:2, matchSubset:1, autoFill:true, matchContains:1, cacheLength:10,  " +
...
<%
        ...
        if (fieldName.indexOf("identifier_citation") > 0) {
            doTextArea(out, item, fieldName, dcSchema, dcElement, dcQualifier,
                    ...
                    closedVocabulary, collectionID, 45);
        ...
        else if (inputType.equals("textarea"))
        {
            doTextArea(out, item, fieldName, dcSchema, dcElement, dcQualifier,
                    repeatable, required, readonly, fieldCountIncr, label, pageContext, vocabulary,
                    closedVocabulary, collectionID, 65);
        }
        ...
        {
            doThreeLocBox(out, item, fieldName, dcSchema, dcElement, dcQualifier,
                    repeatable, required, readonly, fieldCountIncr, label, pageContext,
                    vocabulary, closedVocabulary);
        }
